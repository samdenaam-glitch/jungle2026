...
class GameEntity {
    ...
    update(deltaTime) {
        ...
        // Apply physics if game and physics exist
        if (this.game && this.game.physics && !this.ignoreGravity) {
            ...
            // Simple fallback physics
            this.velocity.y += 0.5; // gravity
            this.x += this.velocity.x * deltaTime * 0.016;
            this.y += this.velocity.y * deltaTime * 0.016;
        }
        
        // Keep within world bounds
        if (this.game && this.game.levelData) {
    ...
    render(ctx) {
        ...
        // Calculate screen position with safety check
        let screenX = this.x;
        let screenY = this.y;
        
        if (this.game && this.game.camera) {
            screenX = this.x - (this.game.camera.x || 0);
            screenY = this.y - (this.game.camera.y || 0);
        }
...
class Player extends GameEntity {
    ...
    canUseAbility(abilityName) {
        ...
        return this.game && this.game.stats && this.game.stats.quantumEnergy >= ability.cost && 
               ability.cooldown <= 0;
    }
    ...
    useQuantumAbility(abilityName) {
        ...
        switch(abilityName) {
            case 'scan':
                if (this.game.audio) this.game.audio.playSound('quantum-scan');
            ...
            case 'timeJump':
                if (this.game.audio) this.game.audio.playSound('time-jump');
            ...
            case 'entangle':
                if (this.game.audio) this.game.audio.playSound('entangle');
                
                // Link nearby collectibles for bonus
                let linkedCount = 0;
                if (this.game.collectibles && this.game.physics) {
                    this.game.collectibles.forEach(item => {
    ...
    render(ctx) {
        ...
        if (this.game && this.game.camera) {
            screenX = this.x - this.game.camera.x;
            screenY = this.y - this.game.camera.y;
        }
        ...
        const timeline = this.game && this.game.stats ? this.game.stats.timeline : 2026;
        switch(timeline) {
            case 1994:
    ...
    update(deltaTime) {
        ...
        switch(this.type) {
            ...
            case 'spikes':
                ...
                if (this.game && this.game.physics && this.game.player && 
                    this.game.physics.checkCollision(this, this.game.player)) {
                    if (this.game.playerTakeDamage) {
...
class Enemy extends GameEntity {
    ...
    takeDamage(amount) {
        ...
        // Visual feedback
        if (this.game && this.game.createParticles) {
            this.game.createParticles(
                this.x + this.width/2,
                this.y + this.height/2,
                10,
...
// PARTICLE ENTITY - BULLETPROOF VERSION
class Particle extends GameEntity {
    constructor(game, x, y, vx, vy, color, life) {
        // Always call super with game instance first
        super(game || window.game, x, y, 4, 4);
        this.velocity.x = vx;
        this.velocity.y = vy;
    ...
    render(ctx) {
        ...
        // Calculate screen position with safety checks
        let screenX = this.x;
        let screenY = this.y;
        
        // Try to get camera position, but handle missing game/camera gracefully
        if (this.game && this.game.camera) {
            screenX = this.x - (this.game.camera.x || 0);
            screenY = this.y - (this.game.camera.y || 0);
        }
